...
│   ├── NpcAI            NPC AI — 角色对话/第3次对话给道具
│   ├── ProtagonistAI    柚子AI — 陪伴回应/物品给予/好感度
│   └── PuzzleAI         谜题AI — 多轮解谜/成功失败判定
...
| **NpcAI**         | NPC角色对话、第3次对话给道具    | `@NPC名 消息` 格式       | 最近30条全类型消息      | `npc`           |
| **PuzzleAI**      | 多轮解谜交互、成功/失败判定      | 有活跃谜题时每回合           | 谜题专属对话记忆(最多20轮) | `puzzle`        |
...
| NPC交互 | `npcMentionPattern` | `@NPC名 消息内容`                                                   | NPC对话 + 自动解锁 + 第3次给道具   |
...
```json
{
  ...
  "revelation": 23,
  "affection": 45,
  "inventory": ["item_pipe_sample", "item_frequency_recording"],
  "foundItems": ["item_pipe_sample", "item_frequency_recording", "item_cable_analysis"],
  ...
  "endingType": null,
  "score": 152,
  "itemNames": { "item_pipe_sample": "管道样本", ... },
...
4. 第3次对话自动给道具机制会自动生效（道具ID模板和触发阈值在 `game_config.json` 的 `npcGiftItemIdTemplate` / `npcGiftDialogueThreshold` 中配置）

### 添加新谜题

1. 在 `puzzles.json` 中添加谜题配置
2. 设置 `difficulty`（1\~5，影响揭露度奖励）
3. 设置 `requiredItemId`（如需前置物品）
...
| 自动拾取上限      | `game_config.json`                 | `autoPickupLimit`               | 2                                      |
